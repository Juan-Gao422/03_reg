...
reg_dfw <- read.csv("../Data/02_reg.csv") %>%
  janitor::clean_names() %>% # clean_names() standardize uppercase, lowercase, and the like.
  mutate(rep = as.factor(rep)) # <dbl> represents number, but rep is not numeric (rep 4 dos not mean double of rep 2), so we want to change it to factor <fctr>.
...
mod1_int <- lmer(yield_mgha ~ 1 + (1 | rep),
                 data = reg_dfw) # 1 = only intercept + random effect coming from blocks
...
mod4_linp <- nlme(yield_mgha ~ SSlinp(sr_ksha,
                                      a, b, xs), # a is intercept, b is the slope, the break point
                  data = reg_dfw,
